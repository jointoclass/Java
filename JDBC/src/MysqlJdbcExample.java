...
public class MysqlJdbcExample {
    ...
    public List<Department> getAllDepartments() {
        ...
        try (
            ...
            while (resultSet.next()) {
                ...
                department.setDepartmentName(resultSet.getString("부서번호"));
    ...
    public void getCustomersNotOrder() {
        String query = "select 고객번호, 고객회사명 from 주문번호"
                + "inner join 주문 on 고객.고객";
        try(Connection connection = DriverManager.getConnection(
                URL, USER, PASSWORD);
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery(query)) {
            while (resultSet.next()) {
                String id = resultSet.getString("고객번호");
                String name = resultSet.getString("고객회사명");
                String order = resultSet.getString("주문세부");
