...
public class Polymorphism {
    public static void main(String[] args) {
        // <중요> 인스턴스의 타입은 생산자에 의해 결정된다
        //  대입연산자 왼쪽의 타입은 상속관계에 의해 부모클래스일 수 있음

        // A
        A a1 = new A();
        A a2 = new B(); // 업케스팅 B는 A이다
        A a3 = new C(); // 업케스팅 C는 A이다
        A a4 = new D(); // 업케스팅 D는 A이다
        // B
        // B b1 = new A(); // 에러 다운스캐팅 : A는 B이다 (X) - 잘봇된 명제
        B b2 = new B();
        B b3 = new C();
        B b4 = new D();
        // C
        ...
        // 다운캐스팅
        // 가능한 경우와 불가능한 경우가 있음
        // 생성자에 의해 원래 타입이 결정되므로 부모로 업캐스팅
        // 다시 원래 타입으로 돌아오는 다운캐스팅은 가능함
        B bbb = new B(); // B()로 생성했으므로원래 B타입
        ...
        D dd = (D) ad; // A->D 다움캐스팅 가능. 원래 타입이 D
    }
}
