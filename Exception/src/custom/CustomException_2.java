...
1) MyRTException의 경우,
예외처리(catch)를 하지 않고 call 매서드가 throws하지 않아도 JVM이 예외처리를 해줌
(JVM의 예외 처리는 catch문과는 다름)
==>자마 컴파일러가 RuntimeException 계열의 경우, 예외처리를 강제하지않도록
설계되어 있다고 함!! (설계철학의 문제)
2) MyException의 경우,
예외처리(catch)를 하지 않고 call 매서드가 throws하지 않으면 컴파일 에러가 발생함
반드시!! 콜스택내의 모든 메서드들이 throws 해야함(main()도 throws가능)
 */
class C {
